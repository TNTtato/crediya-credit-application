...
public class ListApplicationsManualReviewUseCase {
    ...
    public Mono<ItemsPage<ApplicationReviewItem>> execute(String filter, int page, int size) {
        ...
        return stateRepository.findByName(filter)
                ...
                .flatMap(s -> getItemsPageMono(page, size, s, itemsPage));
    }

    private Mono<ItemsPage<ApplicationReviewItem>> getItemsPageMono(int page, int size, State s, ItemsPage<ApplicationReviewItem> itemsPage) {
